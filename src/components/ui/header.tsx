...
export default function Header({ onMenuToggle, isDark = false, selectedLanguage = 'en', onLanguageClick }: HeaderProps) {
  const [showLanguageGlow, setShowLanguageGlow] = useState(false);

  // Site açılışında dil butonuna 10 saniye boyunca glow effect uygula
  useEffect(() => {
    // Component mount olduğunda glow'u başlat
    setShowLanguageGlow(true);
    
    // 10 saniye sonra glow'u kapat
    const glowTimer = setTimeout(() => {
      setShowLanguageGlow(false);
    }, 10000);
  ...
  return (
    <>
      {/* CSS Keyframes for pulsing glow effect */}
      <style jsx>{`
        @keyframes pulseGlow {
          0% {
            box-shadow: 0 0 4px 1px rgba(59, 130, 246, 0.2), 0 0 8px 1px rgba(59, 130, 246, 0.1);
          }
          50% {
            box-shadow: 0 0 12px 2px rgba(59, 130, 246, 0.4), 0 0 20px 3px rgba(59, 130, 246, 0.2);
          }
          100% {
            box-shadow: 0 0 4px 1px rgba(59, 130, 246, 0.2), 0 0 8px 1px rgba(59, 130, 246, 0.1);
          }
      ...
      >
        <div className="container mx-auto px-3 md:px-6 lg:px-8 flex justify-between items-center">
          ...
          <button
            ...
            } ${showLanguageGlow ? 'language-glow' : ''}`}
